...
- Réinitialiser l'interface et la mémoire d'exécution en un clic via un nouveau bouton "Réinitialisation globale", ainsi qu'automatiquement au démarrage (rafraîchissement profond).
- Sélectionner une archive existante via le système de fichiers pour auto-remplir le nom de l'archive (nécessaire pour activer la sélection du dossier source).
...
5. **Vérification Ergonomie / Logique Boutons :**
   - Au démarrage (ou après F5), vérifier que le bouton "SÉLECTIONNER DOSSIER SOURCE" est grisé/désactivé.
   - Cliquer sur "📂 Ou sélectionner une archive existante" et choisir un fichier zip.
   - Constater que le champ "Nom de l'archive" se remplit automatiquement, que la fusion est sélectionnée, et que le bouton de dossier source devient actif.
